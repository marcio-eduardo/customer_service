// Localização sugerida: src/pages/ViewClientsPage/ViewPFClientsPage.tsx
...
interface ViewPFClientsPageProps {
  isDarkMode: boolean;
  // Não precisa mais de clientType, pois este componente é específico para PF
}
...
export function ViewPFClientsPage({ isDarkMode }: ViewPFClientsPageProps) {
  ...
  const apiUrl = 'http://localhost:8080/api/clientes-pf';
  ...
  useEffect(() => {
    const fetchPFClients = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(`Erro HTTP ${response.status} ao buscar clientes PF`);
        }
        
        const data: PaginatedResponse<ClientePfType> = await response.json();
    ...
    fetchPFClients();
  }, [apiUrl]); // apiUrl é constante, mas mantido para clareza ou futuras modificações
