...
public class User {
    ...
    @NotBlank
    @Size(max = 100)
    @Email // Valida se o formato é de email
    @Column(nullable = false)
    private String email;

    @NotBlank
    @Size(max = 120) // O tamanho deve ser suficiente para a password codificada
    @Column(nullable = false)
    private String password;

    @ManyToMany(fetch = FetchType.LAZY) // Os papéis são carregados apenas quando explicitamente solicitados
